...
export function equipmentOptions(skillId) {
  ...
  for (let level = 1; level <= Math.max(profile.reforgeMax, profile.echoMax); level++) {
    const aliases = [];
    if (level <= profile.reforgeMax) aliases.push(`reforge:${level}`);
    if (level <= profile.echoMax) aliases.push(`echo:${level}`);
    options.push({ id: aliases[0], aliases, name: `裝備/回音加成 ${level} 級 ×${1 + level / 10}`, multiplier: 1 + level / 10 });
  }
...
export function calculateSkillBonuses(skillId, settings) {
  ...
  const equipment = equipmentOptions(skillId).find(e => e.id === settings.equipment || e.aliases?.includes(settings.equipment));
