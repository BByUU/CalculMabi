...
test('裝備與回音同級加成合併為十個等級選項，倍率與原選項相同',()=>{
  for(const skill of ['blacksmith','tailoring','magic-craft','hillwen-engineering','handicraft','fynn-craft']) {
    const options=equipmentOptions(skill).filter(o=>o.aliases);
    assert.equal(options.length,10);
    options.forEach((option,i)=>{
      assert.equal(option.name,`裝備/回音加成 ${i+1} 級 ×${1+(i+1)/10}`);
      for(const alias of option.aliases) assert.equal(calculateSkillBonuses(skill,{...base(),equipment:alias}).value,option.multiplier);
    });
  }
});
